...
namespace no_pid {
    ...
    const T make_spill_from_slice (
        ...
        return T([=](const caf::SRSpillProxy *spill) -> A {
            ...
            if (slice_with_longest_track < 0 || N_prim_true < 0) 
                return nopid_slice_var(nullptr, M_prim_true);

            return slice_var(&(spill->slc[slice_with_longest_track]), N_prim_true);
        });
    ...
    int longest_pfp (const caf::SRSliceProxy *slice) {
        ...
        int longest_ipfp = -1;
        for (std::size_t ipfp=0; ipfp<slice->npfp; ++ipfp) {
            if (slice->pfp[ipfp].trk.len > longest_track) {
                longest_track = slice->pfp[ipfp].trk.len;
                longest_ipfp = ipfp;
            }
    ...
    std::vector<int> ordered_pfps_by_length (const caf::SRSliceProxy *slice, const int& N) {
        ...
        for (std::size_t ipfp=0; ipfp<slice->npfp; ++ipfp) {
            if (!std::isnan(slice->pfp[ipfp].trk.len))
                ipfp_length_pairs_all.emplace_back(ipfp, slice->pfp[ipfp].trk.len);
                // safety check
        } // loop ipfp<slice->npfp

        std::sort(ordered_ipfp.begin(), ordered_ipfp.end(), 
            [](const std::pair<int, double> &a, const std::pair<int, double> &b) { return a.second > b.second; }
        );

        if (slice->npfp < N || ordered_ipfp.size() < N) 
            return ordered_ipfp
    ...
    namespace vars {
        double slice_muon_hit_completeness ([](const caf::SRSliceProxy *slice, const int& N) -> double {
        ...
        double slice_leading_proton_hit_completeness ([](const caf::SRSliceProxy *slice, const int& N) -> double {
            if (slice == nullptr) return std::numeric_limits<double>::min();
        ...
        }); // const ana::Var slice_leading_proton_hit_completeness
        
        std::vector<double> slice_protons_hit_completeness ([](const caf::SRSliceProxy *slice, const int& N) -> std::vector<double> {
            ...
            return values;
        }); // const ana::Var slice_protons_hit_completeness
        
        double slice_muon_hit_purity ([](const caf::SRSliceProxy *slice, const int& N) -> double {
            if (slice == nullptr) return std::numeric_limits<double>::min();
            int ipfp_muon = longest_pfp(slice);
        ...
        double slice_leading_proton_hit_purity ([](const caf::SRSliceProxy *slice, const int& N) -> double {
            if (slice == nullptr) return std::numeric_limits<double>::min();
            std::vector<int> ipfps = ordered_pfps_by_length(slice, N);
            int ipfp_proton = ipfps.at(1);

            return slice->reco.pfp.at(ipfp_proton).trk.truth.bestmatch.hit_purity;
        }); // const ana::Var slice_leading_proton_hit_purity

        std::vector<double> slice_protons_hit_purity ([](const caf::SRSliceProxy *slice, const int& N) -> std::vector<double> {
            if (slice == nullptr) return std::vector<double>();
            std::vector<int> ipfps = ordered_pfps_by_length(slice, N);
            ...
            return values;
        }); // const ana::Var slice_protons_hit_purity

        double slice_muon_L_reco_true_ratio ([](const caf::SRSliceProxy *slice, const int& N) -> double {
            if (slice == nullptr) return std::numeric_limits<double>::min();
            int ipfp_muon = longest_pfp(slice);
        ...
        double slice_leading_proton_L_reco_true_ratio ([](const caf::SRSliceProxy *slice, const int& N) -> double {
            if (slice == nullptr) return std::numeric_limits<double>::min();
            std::vector<int> ipfps = ordered_pfps_by_length(slice, N);
            int ipfp_proton = ipfps.at(1);
        ...
        std::vector<double> slice_protons_L_reco_true_ratio ([](const caf::SRSliceProxy *slice, const int& N) -> std::vector<double> {
            if (slice == nullptr) return std::vector<double>();
            std::vector<int> ipfps = ordered_pfps_by_length(slice, N);
        ...
        double slice_vertex_delta_3D ([](const caf::SRSliceProxy *slice, const int& N) -> double {
            if (slice == nullptr) return std::numeric_limits<double>::min();
            if (
        ...
        double slice_vertex_delta_x ([](const caf::SRSliceProxy *slice, const int& N) -> double {
            if (slice == nullptr) return std::numeric_limits<double>::min();
            if (
                std::isnan(slice->vertex.x) ||
                std::isnan(slice->truth.position.x) 
        ...
        double slice_vertex_delta_y ([](const caf::SRSliceProxy *slice, const int& N) -> double {
            if (slice == nullptr) return std::numeric_limits<double>::min();
            if (
                std::isnan(slice->vertex.y) ||
                std::isnan(slice->truth.position.y) 
        ...
        double slice_vertex_delta_z ([](const caf::SRSliceProxy *slice, const int& N) -> double {
            if (slice == nullptr) return std::numeric_limits<double>::min();
            if (
                std::isnan(slice->vertex.z) ||
                std::isnan(slice->truth.position.z) 
        ...
        double slice_muon_P_reco_true_ratio ([](const caf::SRSliceProxy *slice, const int& N) -> double {
            if (slice == nullptr) return std::numeric_limits<double>::min();
            int ipfp_muon = longest_pfp(slice);

            return slice->reco.pfp.at(ipfp_muon).trk.rangeP.p_muon / slice->reco.pfp.at(ipfp_muon).trk.truth.p.startp.Mag();
        }); // const ana::Var slice_muon_L_reco_true_ratio

        double slice_leading_proton_P_reco_true_ratio ([](const caf::SRSliceProxy *slice, const int& N) -> double {
            if (slice == nullptr) return std::numeric_limits<double>::min();
            std::vector<int> ipfps = ordered_pfps_by_length(slice, N);
            int ipfp_proton = ipfps.at(1);

            return slice->reco.pfp.at(ipfp_proton).trk.rangeP.p_proton / slice->reco.pfp.at(ipfp_proton).trk.truth.p.startp.Mag();
        }); // const ana::Var slice_leading_proton_L_reco_true_ratio

        std::vector<double> slice_protons_P_reco_true_ratio ([](const caf::SRSliceProxy *slice, const int& N) -> std::vector<double> {
            if (slice == nullptr) return std::vector<double>();
            std::vector<int> ipfps = ordered_pfps_by_length(slice, N);
            ...
            for (std::size_t i=1; i<ipfps.size(); i++) 
                values.push_back(
                    slice->reco.pfp.at(ipfps.at(i)).trk.rangeP.p_proton / slice->reco.pfp.at(ipfps.at(i)).trk.truth.p.startp.Mag();
                );

            return values;
        }); // const ana::Var slice_protons_L_reco_true_ratio

        double slice_neutrino_reco_E ([](const caf::SRSliceProxy *slice, const int& N) -> double {
            if (slice == nullptr) return std::numeric_limits<double>::min();
            int ipfp_muon = longest_pfp(slice);
        ...
        double slice_neutrino_true_E ([](const caf::SRSliceProxy *slice, const int& N) -> double {
            if (slice == nullptr) return std::numeric_limits<double>::min();
        ...
        double slice_neutrino_reco_dE ([](const caf::SRSliceProxy *slice, const int& N) -> double {
            if (slice == nullptr) return std::numeric_limits<double>::min();
            int ipfp_muon = longest_pfp(slice);
        ...
        double slice_neutrino_reco_pT ([](const caf::SRSliceProxy *slice, const int& N) -> double {
            if (slice == nullptr) return std::numeric_limits<double>::min();
            int ipfp_muon = longest_pfp(slice);
