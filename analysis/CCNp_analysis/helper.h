...
namespace hard_code {
    ...
    bool is_bad_event(const caf::SRSpillProxy *sr) {
...
namespace cheating {
    ...
    const ana::SpillMultiVar test_variables ([](const caf::SRSpillProxy *spill) -> std::vector<double> {
        ...
        for (auto const& slice: spill->slc) {
            ...
            std::cout << "Looking at true info" << std::endl;
            ...
            for (auto const& prim: slice.truth.prim) {
                ...
                if (prim.daughters.size() > 0) {
                    ...
                    for (auto const& true_particle: spill->true_particles) {
                        if (true_particle.parent == prim.G4ID)
                            std::cout << "\tdaughter #: " << id << ", " 
                                      << "pdg = " << true_particle.pdg << ", "
                                      << "len = " << true_particle.length << " cm, "
                                      << "start = (" << true_particle.start.x << ", " 
                                      << true_particle.start.y << ", " 
                                      << true_particle.start.z << "), "
                                      << "end = (" << true_particle.end.x << ", " 
                                      << true_particle.end.y << ", " 
                                      << true_particle.end.z << "), "
