...
namespace particle_data {
    ...
    struct int_type_t {
        int num_protons_above50;
        int num_muons;
        int num_pions;
        int num_neutral_pions;
        int num_gamma;
        double length_muon;
        double length_short_proton;
        bool all_contained;
        bool unclassified;
    };
    ...
    const R make_spill_from_slice (
        ...
        return R([=](const caf::SRSpillProxy *spill) -> A {
            ...
            for (auto const& slice: spill->slc) {
                ...
                particle_data::int_type_t interaction = classification_type(spill, &slice);
                ...
                if (what_to_cut_on == cut_type_t::TRUE_1muN1p && !(
                    interaction.num_protons_above50 > 1 &&
                ...
                if (what_to_cut_on == cut_type_t::TRUE_1mu2p && !(
                    interaction.num_protons_above50 == 2&&
                    interaction.num_muons == 1          &&
                    interaction.num_pions == 0          &&
                    interaction.num_neutral_pions == 0  &&
                    interaction.num_gamma == 0          &&
                ...
                if (what_to_cut_on == cut_type_t::TRUE_1mu3p && !(
                    interaction.num_protons_above50 == 3&&
                    interaction.num_muons == 1          &&
                    interaction.num_pions == 0          &&
                    interaction.num_neutral_pions == 0  &&
                    interaction.num_gamma == 0          &&
...
namespace cuts {
    ...
    namespace reco {
        ...
        const ana::Cut slice_Np_only ([](const caf::SRSliceProxy *slice) -> bool {
            int ipfp_muon = var_utils::find_muon(*slice, var_utils::dist_cut);
            if (ipfp_muon == -1) return false; // redundant, btw, but who cares...
...
namespace var_utils {
    particle_data::int_type_t classification_type (const caf::SRSpillProxy *spill, const caf::SRSliceProxy *slice) {
        ...
        particle_data::int_type_t interactions;
        interactions.num_protons_above50 = 0;
        interactions.num_muons = 0;
        interactions.num_pions = 0;
        ...
        if (std::isnan(slice->vertex.x) || std::isnan(slice->vertex.y) || std::isnan(slice->vertex.z)) {
            interactions.unclassified = true;
            return interactions;
        }
        ...
        interactions.num_protons_above50 = num_protons_above50;
        interactions.num_muons = num_muons;
        interactions.num_pions = num_pions;
        interactions.num_neutral_pions = num_neutral_pions;
