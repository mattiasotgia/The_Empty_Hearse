...
void huntingMichels () {
  ...
  std::map<std::string, std::string> availableLoaders = {
    {"nominal", "msotgia_v09_89_01_01p03_bdtCheatingMichel_nominal"},
    {"cheated", "msotgia_v09_89_01_01p03_bdtCheatingMichel_cheated"},
  };

  std::vector<std::string> runningLoaders = {"nominal", "cheated"};
  ...
  for (auto const& runningLoader: runningLoaders) {
    ...
    treesElectrons.emplace_back(std::make_unique<ana::Tree>( 
      runningLoader.c_str(),
      std::vector<std::string>{"E", "score"},
      loader,
      std::vector<ana::SpillMultiVar>{E, score},
      ana::kNoSpillCut,
      true
    ));
