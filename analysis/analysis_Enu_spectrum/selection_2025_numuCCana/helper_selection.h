...
bool all_contained(const caf::Proxy<caf::SRSlice> &islc)
{
    ...
    {
        if (std::isnan(islc.reco.pfp[ipfp].trk.start.x) || std::isnan(islc.reco.pfp[ipfp].trk.end.x) || std::isnan(islc.reco.pfp[ipfp].trk.len))
            continue;
        ...
        if ((islc.reco.pfp[ipfp].trk.start.x * islc.vertex.x) < 0)
        {
            return false;
        } // not contained if they cross cryostats
        // if not contained return false
        if (!isInContained(islc.reco.pfp[ipfp].trk.end.x, islc.reco.pfp[ipfp].trk.end.y, islc.reco.pfp[ipfp].trk.end.z, 5.0))
        {
            return false;
        }
    }
...
int find_muon(const caf::Proxy<caf::SRSlice> &islc, int dist_mucut)
{
    ...
    for (std::size_t ipfp(0); ipfp < islc.reco.npfp; ++ipfp)
    {
        if (std::isnan(islc.reco.pfp[ipfp].trk.start.x) || std::isnan(islc.reco.pfp[ipfp].trk.len))
            continue;
        // if(!isInDetector(islc.reco.pfp[ipfp].trk.start.x,islc.reco.pfp[ipfp].trk.start.y,islc.reco.pfp[ipfp].trk.start.z))continue;
        // if(!isInDetector(islc.reco.pfp[ipfp].trk.end.x,islc.reco.pfp[ipfp].trk.end.y,islc.reco.pfp[ipfp].trk.end.z))continue;
...
int id_pfp(const caf::Proxy<caf::SRSlice> &islc, int ipfp, int dist_cut)
{
    ...
    if (islc.reco.pfp[ipfp].trackScore >= 0.5)
    {
        if (std::isnan(islc.reco.pfp[ipfp].trk.start.x) || std::isnan(islc.reco.pfp[ipfp].trk.end.x) || std::isnan(islc.reco.pfp[ipfp].trk.len))
            return 9;
        if (std::isnan(islc.reco.pfp[ipfp].trk.start.y) || std::isnan(islc.reco.pfp[ipfp].trk.start.z) || std::isnan(islc.reco.pfp[ipfp].trk.end.y) || std::isnan(islc.reco.pfp[ipfp].trk.end.z))
            return 9;
        ...
        if (output[1] >= 100 && ((RecoVtx - start).Mag() < dist_cut) && (sqrt(pow(139.570, 2) + pow(Start_mom_v.Mag() * 1000, 2)) - 139.570) >= 25.0)
        {
            if (islc.reco.pfp[ipfp].parent_is_primary)
            {
                return 2;
        ...
        if (output[1] < 100 && ((RecoVtx - start).Mag() < dist_cut) && (sqrt(pow(938.3, 2) + pow(Start_mom_v.Mag() * 1000, 2)) - 938.3) >= 50.0)
        {
            if (islc.reco.pfp[ipfp].parent_is_primary)
            {
                return 1;
    ...
    if (islc.reco.pfp[ipfp].trackScore < 0.5)
    {
        if (islc.reco.pfp[ipfp].trackScore >= 0.4 && islc.reco.pfp[ipfp].trackScore < 0.5 && output[1] < 100)
        {
            ...
            if ((sqrt(pow(938.3, 2) + pow(Start_mom_v2.Mag() * 1000, 2)) - 938.3) >= 50.0 && ((RecoVtx - start).Mag() < dist_cut) && islc.reco.pfp[ipfp].parent_is_primary)
            {
                return 1;
            }
        }
        if (!(islc.reco.pfp[ipfp].trackScore >= 0.4 && islc.reco.pfp[ipfp].trackScore < 0.5 && output[1] < 100))
        {
            ...
            int use_plane2 = 2;
            if (std::isnan(islc.reco.pfp[ipfp].shw.plane[use_plane2].energy))
                return 9;
            if (islc.reco.pfp[ipfp].shw.plane[use_plane2].energy * 1000 < 25.0)
                return 9;
            if (islc.reco.pfp[ipfp].shw.plane[use_plane2].energy * 1000 > 25.0)
            {
                if (islc.reco.pfp[ipfp].parent_is_primary)
                            ...
                            if (all_contained(islc))
                            {
                                ...
                                if (ipfp_mu != -1)
                                {
                                    ...
                                    for (std::size_t ipfp(0); ipfp < islc.reco.npfp; ++ipfp)
                                    {
                                        ...
                                        // if(!isInDetector(islc.reco.pfp[ipfp].trk.start.x,islc.reco.pfp[ipfp].trk.start.y,islc.reco.pfp[ipfp].trk.start.z))continue;
                                        // if(!isInDetector(islc.reco.pfp[ipfp].trk.end.x,islc.reco.pfp[ipfp].trk.end.y,islc.reco.pfp[ipfp].trk.end.z))continue;
                                        if (id_pfp(islc, ipfp, dist_smucut) == 1)
                                        {
                                            num_protons += 1;
                                        }
                                        if (id_pfp(islc, ipfp, dist_smucut) == 2)
                                        {
                                            num_pions += 1;
                                        }
                                        if (id_pfp(islc, ipfp, dist_smucut) == 3)
                                        {
                                            num_showers += 1;
                                        }
                                    }
                        ...
                        if (all_contained(islc))
                        {
                            ...
                            if (ipfp_mu != -1)
                            {
                                ...
                                for (std::size_t ipfp(0); ipfp < islc.reco.npfp; ++ipfp)
                                {
                                    ...
                                    // if(!isInDetector(islc.reco.pfp[ipfp].trk.start.x,islc.reco.pfp[ipfp].trk.start.y,islc.reco.pfp[ipfp].trk.start.z))continue;
                                    // if(!isInDetector(islc.reco.pfp[ipfp].trk.end.x,islc.reco.pfp[ipfp].trk.end.y,islc.reco.pfp[ipfp].trk.end.z))continue;
                                    if (id_pfp(islc, ipfp, dist_cut) == 1)
                                    {
                                        num_protons += 1;
                                    }
                                    if (id_pfp(islc, ipfp, dist_cut) == 2)
                                    {
                                        num_pions += 1;
                                    }
                                    if (id_pfp(islc, ipfp, dist_cut) == 3)
                                    {
                                        num_showers += 1;
                                    }
                                }
...
int classification_type(const caf::SRSpillProxy *sr, const caf::Proxy<caf::SRSlice> &islc)
{
    ...
    if (std::isnan(islc.vertex.x) || std::isnan(islc.vertex.y) || std::isnan(islc.vertex.z))
        return 99;
    if (std::isnan(islc.truth.position.x) || std::isnan(islc.truth.position.y) || std::isnan(islc.truth.position.z))
        return 99;
        ...
        isInActive(islc.truth.position.x, islc.truth.position.y, islc.truth.position.z))
    {
        if (isInFV(islc.truth.position.x, islc.truth.position.y, islc.truth.position.z))
        {
            ...
            for (auto const &ipart : islc.truth.prim)
            {
                ...
                if (abs(ipart.pdg) == 111)
                { // Neutral pions - reject if any of its gamma > 25 MeV
                    if (ipart.daughters.size() > 0)
                    {
                ...
                if (abs(ipart.pdg) == 22 && dep_E > 25.0)
                {
                    return 3;
                } // primary photons
                ...
                if (abs(ipart.pdg) == 2212 && dep_E > 50.0)
                {
            ...
            {
                if (length_muon > 50. && all_contained_truth(sr, islc))
                {
                    return 2;
                }
            }
...
int classification_type_MC(const caf::SRSpillProxy *sr, const caf::Proxy<caf::SRTrueInteraction> &nu)
{
        ...
        isInActive(nu.position.x, nu.position.y, nu.position.z))
    {
        if (isInFV(nu.position.x, nu.position.y, nu.position.z))
        {
            ...
            if (num_muons == 1 && num_protons_above50 == 1)
            {
                if (length_muon2 > 50. && all_contained_mc(sr, nu))
                {
                    return 2;
                }
            }
...
int good_runs[305] = {9301,9302,9303,9307,9308,9309,9310,9311,9312,9313,9314,9315,9316,9317,9318,9327,9328,9329,9330,9331,9333,9334,9335,9336,9337,9338,9339,9340,9341,9342,9343,9344,9345,9346,9347,9353,9354,9355,9356,9357,9358,9359,9360,9361,9362,9363,9364,9365,9366,9367,9368,9369,9370,9371,9372,9373,9374,9375,9376,9377,9378,9379,9380,9383,9384,9385,9386,9387,9388,9389,9390,9391,9392,9393,9394,9409,9412,9417,9418,9419,9420,9421,9422,9423,9424,9425,9426,9427,9430,9431,9432,9435,9436,9437,9438,9439,9441,9444,9445,9448,9450,9457,9458,9460,9464,9472,9473,9474,9475,9477,9478,9481,9482,9495,9499,9504,9515,9516,9517,9518,9531,9533,9534,9558,9560,9562,9563,9564,9565,9568,9569,9570,9582,9583,9584,9585,9586,9587,9588,9589,9590,9593,9594,9595,9597,9598,9602,9610,9624,9625,9626,9627,9631,9634,9642,9646,9647,9648,9649,9658,9672,9673,9674,9675,9687,9688,9689,9690,9691,9692,9693,9694,9695,9696,9697,9698,9699,9700,9703,9704,9705,9714,9715,9716,9717,9720,9721,9722,9723,9724,9725,9726,9727,9728,9729,9730,9731,9732,9733,9734,9735,9743,9744,9745,9747,9748,9749,9750,9751,9752,9753,9754,9755,9756,9757,9758,9760,9761,9762,9763,9764,9765,9766,9779,9780,9782,9783,9785,9788,9791,9792,9793,9794,9795,9796,9797,9807,9814,9834,9835,9836,9837,9838,9840,9841,9848,9850,9858,9859,9861,9863,9867,9869,9871,9893,9911,9915,9918,9920,9923,9929,9931,9932,9933,9934,9935,9940,9943,9947,9948,9955,9961,9962,9968,9973,9975,9978,9980,9983,9985,9987,9988,9989,9991,10009,10010,10015,10030,10040,10048,10055,10056,10058,10062,10063,10067,10068,10088,10089,10090,10091,10092,10093,10095,10098};
